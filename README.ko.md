...
2. **아이콘 적용** — 탐색기에서 파일이나 폴더를 우클릭하고 **Change custom icon**을 선택한 뒤 라이브러리에서 아이콘을 선택하세요.
3. **인라인 아이콘** — 설정에서 인라인 아이콘을 활성화한 뒤, 노트에 `:custom-icon-이름:`을 입력하세요 (`이름`은 라이브러리에 등록된 아이콘 이름).
